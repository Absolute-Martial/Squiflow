...
- The implemented OpenFGA surface covers authenticated tenant-workspace read plus the narrow Order draft create/read slice. Finbuckle resolves the route tenant only as an untrusted candidate; current membership creates `TenantContext`; the pinned model then intersects contextual membership with the relevant persisted `workspace_viewer`, `order_creator`, or `order_viewer` relation at `HIGHER_CONSISTENCY`. Provider timeout/error fails closed. This does not introduce role or tuple-administration workflows.
- The first tenant-owned business persistence is an immutable priced Order draft create/read slice. It uses explicit tenant SQL predicates plus forced PostgreSQL RLS, transaction-local tenant context, and a receipt committed atomically with the order. Create idempotency is scoped by tenant, current account, operation and caller key. This does not settle the broader Orders lifecycle or discovery-sensitive customer/work/job distinctions; `docs/implementation/ORDER_DRAFT_INTAKE_SLICE.md` owns the exact claim.
