...
- The implemented OpenFGA surface covers authenticated tenant-workspace read plus narrow Order draft create/edit/read/browse/abandon operations. Finbuckle resolves the route tenant only as an untrusted candidate; current membership creates `TenantContext`; the pinned model then intersects contextual membership with the relevant persisted `workspace_viewer`, `order_creator`, `order_editor`, `order_viewer`, or `order_abandoner` relation at `HIGHER_CONSISTENCY`. Editing and abandonment each require a separate relation. Provider timeout/error fails closed. This does not introduce role or tuple-administration workflows.
- The first tenant-owned business persistence is a priced Order draft with create/edit/read/browse and one-way abandonment. It uses explicit tenant SQL predicates plus forced PostgreSQL RLS and transaction-local tenant context. Create commits a caller-scoped semantic-idempotency receipt atomically with the order; editing replaces still-open draft content under an expected revision and stores its own caller-scoped receipt; browse uses bounded keyset pagination over stable descending creation/identity order; abandonment requires the current revision, preserves the latest priced content and prior receipts, and commits a separate receipt with the transition. This does not introduce text search or settle the broader Orders lifecycle or discovery-sensitive customer/work/job distinctions; `docs/implementation/ORDER_DRAFT_INTAKE_SLICE.md` owns the exact claim.
...
- The first Web business journey is directed toward a complete commercial order flow, developed step by step across quotation, order, fulfillment, invoicing, payment and relevant credit/return/refund paths, rather than a read-only shell or priced-draft-only pilot. The first concrete case is an organization with program/account billing. These are conditional capabilities, not one mandatory sequence for every tenant. `docs/product/PRODUCT_FOUNDATION.md` owns the still-open actor, start/end condition, exact commercial meanings, evidence, manual steps and product-promise qualification; this direction alone does not introduce new runtime contracts.
- The first implemented step for that case is tenant-owned customer organization and child-program setup plus optional, validated Orders draft attribution. The organization/program records have immutable identity and parentage, but the association is not legal debtor, commercial-account, invoice, credit, or settlement authority. `docs/implementation/CUSTOMER_ORGANIZATION_PROGRAM_ATTRIBUTION_SLICE.md` owns its precise scope and evidence.
- In the first organization/program billing case, the organization is the default invoice debtor. A program can instead owe independently, or its charges can be assigned to an individual account. The eventual issued invoice must retain the explicit bill-to choice; the current draft's `customerContext` is attribution only. `docs/product/PRODUCT_FOUNDATION.md` and `docs/domain/BUSINESS_MODEL.md` own this direction; individual-account identity, assignment authority and settlement rules remain open.
