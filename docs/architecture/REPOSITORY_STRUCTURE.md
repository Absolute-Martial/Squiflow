...
Qualified 0B introduced the first capability/test pair. Active 0E has introduced a second capability/test pair:
...
`SquiFlow.Payments` currently introduces only the documented Payment status vocabulary and its no-outward-dependency boundary. Its claims remain `BLOCKED` until executable verification passes for the active 0E branch.
