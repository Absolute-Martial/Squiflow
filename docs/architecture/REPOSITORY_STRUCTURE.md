...
The current tree contains four compact host-neutral capability projects (`Application.Profiles`, `Application.Branding`, `Application.IdentityAccess`, and `Application.Tenancy`), two capability-owned PostgreSQL adapters, CoreApi and DbMigrator executables, seven test projects and a repository build/test contract. ApplicationProfiles validates bounded feature graphs and compiles deterministic dependency-closed selections, but contains no production catalog or durable profile authority. CoreApi has configured standards-based JWT validation, authenticated account and active-membership queries, Finbuckle route-candidate plumbing, and one pinned-model OpenFGA tenant-workspace read check. Tenancy resolves an immutable context from current membership. The tree contains no Foundation/ApplicationKernel project, broader role/tuple administration, tenant-owned business data/RLS or business capability.
