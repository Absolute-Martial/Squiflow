...
`services/core-api` is the current compact ASP.NET Core composition host. Its implemented scope is public brand bootstrap/liveness, a generated `/openapi/v1.json` contract, configured JWT bearer validation, exact external-identity-to-account resolution, current active-membership listing, authorized tenant-workspace read, tenant-owned customer organization/program create/read/browse, and optionally customer-attributed immutable priced Order draft create/read/browse with one-way abandonment. The API document uses configured public brand identity and describes the configured OpenID Connect discovery authority only on protected operations. The Tenancy capability establishes `TenantContext` only after a current membership check. Workspace, Customers and Orders operations then use typed ASP.NET resource requirements backed by the pinned OpenFGA model. The Orders create and abandon operations require caller-scoped semantic idempotency; abandonment additionally requires its own OpenFGA relation and expected revision. Browse is a bounded newest-first keyset query. They delegate business meaning and persistence to the Customers and Orders capabilities. Their exact scopes are owned by `docs/implementation/CUSTOMER_ORGANIZATION_PROGRAM_ATTRIBUTION_SLICE.md` and `docs/implementation/ORDER_DRAFT_INTAKE_SLICE.md`.
