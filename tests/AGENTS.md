...
The active Payments tests currently claim only:
...
The Payments claims remain `BLOCKED` until executable verification passes for the active 0E branch.
...
Qualified 0B has successful GitHub execution evidence for the then-current two-project solution. Because 0E changes C#, projects, tests, and the solution, that earlier run does not qualify the Payments claims. A new successful execution is required.

GitLab pipeline `#196` for the first 0E code commit failed before start with `ci_quota_exceeded`, `runner = null`; no `dotnet` command executed. Treat this as an infrastructure-capacity limitation, not as a code/test result.

Do not claim these commands pass for the active 0E branch until actual executable evidence exists.
