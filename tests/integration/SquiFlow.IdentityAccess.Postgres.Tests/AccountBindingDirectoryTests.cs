...
public sealed class AccountBindingDirectoryTests : PostgresTestDatabase
{
    ...
    private async Task<string> CreateReadOnlyRuntimeRoleAsync()
    {
        ...
        command.CommandText = """
            CREATE ROLE squiflow_identity_reader LOGIN PASSWORD 'local-runtime-test-only';
            GRANT CONNECT ON DATABASE squiflow_tests TO squiflow_identity_reader;
            GRANT USAGE ON SCHEMA identity_access TO squiflow_identity_reader;
            GRANT SELECT ON ALL TABLES IN SCHEMA identity_access TO squiflow_identity_reader;
            """;
        await command.ExecuteNonQueryAsync(CancellationToken.None);

        var runtime = new NpgsqlConnectionStringBuilder(ConnectionString)
        {
            Username = "squiflow_identity_reader",
