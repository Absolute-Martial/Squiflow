...
public sealed class TenantMembershipDirectoryTests : PostgresTestDatabase
{
    ...
    [Fact]
    public async Task MembershipRequiresAnExistingApplicationAccount()
    {
        await CreateMigrationRunner()
            .ApplyAsync(TimeSpan.FromSeconds(10), CancellationToken.None);
    ...
    private async Task<string> CreateReadOnlyRuntimeRoleAsync()
    {
        ...
        command.CommandText = """
            CREATE ROLE application_tenancy_reader LOGIN PASSWORD 'local-runtime-test-only';
            GRANT CONNECT ON DATABASE application_tests TO application_tenancy_reader;
            GRANT USAGE ON SCHEMA tenancy TO application_tenancy_reader;
            GRANT SELECT ON ALL TABLES IN SCHEMA tenancy TO application_tenancy_reader;
            """;
        await command.ExecuteNonQueryAsync(CancellationToken.None);

        var runtime = new NpgsqlConnectionStringBuilder(ConnectionString)
        {
            Username = "application_tenancy_reader",
