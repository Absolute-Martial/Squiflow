...
active runtime responsibilities: public application bootstrap; classified OpenAPI v1 description; JWT access-token validation; authenticated account resolution; tenant membership listing/context resolution; Finbuckle route-candidate resolution; pinned-model OpenFGA tenant-workspace authorization; account/tenancy persistence; Autofac root composition; bounded dormant profile-runtime mechanics; one-shot DB migration
...
The bounded ApplicationProfiles feature compiler, deployment-wide public Branding contract, CoreApi bootstrap/liveness paths, classified OpenAPI v1 document, configured JWT validation, active-account resolution, current active-membership listing, exact account/membership queries, immutable membership-derived `TenantContext`, Finbuckle route-candidate plumbing, pinned-model OpenFGA tenant-workspace read authorization, shared bounded/resetting CoreApi Npgsql data source, and the one-shot PostgreSQL migrator are `PRODUCTION_HONEST` for their declared narrow scopes. Real ASP.NET pipeline tests prove that the API document uses configured public identity, declares the configured OpenID Connect authority, applies its security requirement only to protected operations and contains no repository codename. They also reject missing, wrongly issued, wrong-audience, expired, incorrectly signed and incomplete identities with generic safe failures. Tenant-workspace pipeline tests prove authentication, membership-before-authorization ordering, cross-account denial, permission denial and safe provider-outage behavior. A real OpenFGA 1.21.0 container test proves the checked-in model denies without the persisted permission tuple, allows the verified member with `workspace_viewer`, and remains pinned after a newer model is written. Configuration tests reject implicit-latest/malformed model identity, remote plaintext endpoints and ignored credentials. Real PostgreSQL tests prove model/migration agreement, ordered multi-capability migration, repeat application, bounded migration locking, uniqueness, exact binding lookup, cross-account membership denial, suspension denial, account referential integrity and read-only runtime roles. Tenant-specific branding, real ZITADEL topology/flows, broader OpenFGA roles/administration, tenant-owned RLS and business capabilities are `NOT_INTRODUCED`. `BLOCKED = none`.
