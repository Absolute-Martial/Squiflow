...
The current service executables are `Application.CoreApi` for public bootstrap/liveness plus configured JWT validation, authenticated account resolution, active tenant-membership listing, and tenant workspace, customer organization/program create/read/browse, and optionally customer-attributed order-draft create/read/browse/revise/abandon protected by current membership plus pinned OpenFGA permission checks; `Application.DatabaseMigrator` applies ordered one-shot IdentityAccess, Tenancy, Customers and Orders PostgreSQL migrations. CoreApi uses Autofac as its root provider, Finbuckle only for untrusted route-candidate resolution, and ASP.NET Core resource authorization with the official OpenFGA SDK for narrow workspace and order checks. It composes the capability adapters over one explicitly bounded/resetting Npgsql data source, requires bounded non-permissive host/trust/cache/resource/authorization configuration, and owns the membership-derived `TenantContext` resolver. The narrow Orders operation is priced draft create/read, bounded cursor browse, full revision of an uncommitted draft, and one-way abandon. Customer organization/program creation and viewing use separate persisted permissions; Order create, view, revise and abandon use distinct `order_creator`, `order_viewer`, `order_editor`, and `order_abandoner` permissions. Customer context on a draft is attribution, not legal billing authority. Abandon requires an expected revision, records the abandoning account and timestamp, increments revision and commits with caller-scoped idempotency; PostgreSQL permits only required editable-header/lifecycle-column updates and draft-only line replacement under explicit tenant predicates and forced RLS. It preserves the original create receipt, and does not add deletion or a broader order lifecycle or text-search surface. CoreApi also contains bounded internal profile-runtime mechanics, which no production path acquires yet. It contains no external PostgreSQL pooler, durable profile authority, implementation variant, role/tuple administration, Worker or broader business lifecycle. DbMigrator requires an explicit deployment advisory-lock identity and timeout; it owns no seeding, per-tenant loop, identity placeholder, scheduler or application startup behavior.
