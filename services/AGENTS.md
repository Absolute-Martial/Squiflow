...
The current service executables are `Application.CoreApi` for public bootstrap/liveness plus configured JWT validation, authenticated account resolution, active tenant-membership listing and a tenant-workspace read protected by current membership plus a pinned OpenFGA permission check, and `Application.DatabaseMigrator` for ordered one-shot IdentityAccess/Tenancy PostgreSQL migration. CoreApi uses Autofac as its root provider, Finbuckle only for untrusted route-candidate resolution, and ASP.NET Core resource authorization with the official OpenFGA SDK for the narrow workspace check. It composes both query adapters over one explicitly bounded/resetting Npgsql data source, requires bounded non-permissive host/trust/cache/resource/authorization configuration, and owns the membership-derived `TenantContext` resolver. It also contains tested internal bounded profile-runtime mechanics that no production path acquires yet. It contains no external PostgreSQL pooler, durable profile authority, implementation variant, role/tuple administration or business mutation. DbMigrator requires an explicit deployment advisory-lock identity and timeout; it owns no seeding, per-tenant loop, identity placeholder, scheduler or application startup behavior.
