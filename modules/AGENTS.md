...
The current capability projects are `Application.Profiles`, `Application.Branding`, `Application.IdentityAccess`, `Application.Tenancy`, `Application.Orders`, and the earned provider-isolated IdentityAccess/Tenancy/Orders PostgreSQL adapters. ApplicationProfiles owns bounded feature definitions, catalog graph validation and deterministic effective-selection compilation; it owns no production feature catalog, durable tenant profile authority, activation, authorization or Autofac integration. Branding owns bounded public deployment identity. IdentityAccess owns stable external `(issuer, subject)` account binding. Tenancy owns the tenant registry, current account membership query and membership-derived `TenantContext`; it owns no OpenFGA role/permission model. Orders owns only immutable priced draft create/read with caller-scoped semantic idempotency; its PostgreSQL adapter owns the narrow tenant-owned draft, line and command-receipt persistence boundary. Broader order lifecycle meaning remains absent. The former `Application.Parties` enum-only slice remains purged.
...
The current unit tests mechanically protect ApplicationProfiles, Branding, IdentityAccess, Tenancy and Orders from ASP.NET, EF/Npgsql, OpenFGA and FSH dependencies. Preserve and extend those checks as capability boundaries grow.
...
Current active claims are limited to bounded feature graph/selection compilation, bounded Branding values, exact external-account binding, current active tenant-membership queries and membership-derived `TenantContext`, plus immutable priced order-draft create/read and caller-scoped semantic idempotency. Provider behavior is additionally tested against real PostgreSQL.
