...
- Interactive authentication uses OpenID Connect; SquiFlow application authorization is separate.
- Workstation login uses system browser + Authorization Code + PKCE `S256` and no embedded reusable client secret.
- Stable external account identity is `(issuer, subject)`, not mutable email.
- Tenant authorization changes advance a versioned authorization revision so stale permission snapshots/caches can be detected.
- ASP.NET Core resource-based authorization is the implementation primitive for object/action checks that depend on loaded data.
- API security gates explicitly cover the OWASP API Security Top 10 categories that apply.
- SquiFlow adopts Zanzibar's authorization-freshness lessons without introducing a Zanzibar service/tuple graph/indexing stack.
...
- [`docs/implementation/PHASES_AND_GATES.md`](docs/implementation/PHASES_AND_GATES.md) — sequential implementation order and source-backed security gates.
...
### Review/quality/source reconciliation
- [`docs/review/SECURITY_AUTHORIZATION_SOURCE_REVIEW.md`](docs/review/SECURITY_AUTHORIZATION_SOURCE_REVIEW.md) — sequential review of OpenID Connect, OWASP API Security Top 10, Zanzibar and ASP.NET Core resource authorization.
...
Earlier ZIPs contained generated CSV inventory/review ledgers. They became stale and could encode old classifications as if they were decisions. GitLab now keeps semantic current decisions in Markdown; machine inventory/hash/API-surface reports can be generated by CI when needed.

## Versioning

The current baseline is `v0.0.15`. Architecture changes update the relevant current documents and decision records. Version increments are deliberate release/baseline decisions rather than being forced by every documentation refinement.
